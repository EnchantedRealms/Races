		...
		foreach($this->plugin->getServer()->getOnlinePlayers() as $p){
			if($this->plugin->hasRace($p->getName()) == true){
				if($this->plugin->getRace($p->getName()) == 0){
					$p->addEffect(Effect::getEffect(Effect::FIRE_RESISTANCE)->setDuration(20)->setAmplifier(2)->setVisible(false));
				}
				...
				if($this->plugin->getRace($p->getName()) == 2){
					$p->addEffect(Effect::getEffect(Effect::REGENERATION)->setDuration(20)->setAmplifier(2)->setVisible(false));
				}
			}
		}
	}
